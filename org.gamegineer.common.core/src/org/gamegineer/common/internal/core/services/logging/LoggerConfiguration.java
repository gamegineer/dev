...
final class LoggerConfiguration
{
        ...
        final Filter defaultValue )
    {
        ...
        if( value != null )
        {
            ...
            {
                Debug.getDefault().trace( Debug.OPTION_SERVICES_LOGGING, String.format( "Illegal filter name specified for logger '%1$s'", name_ ), e ); //$NON-NLS-1$
            }
            catch( final ComponentException e )
            {
                Debug.getDefault().trace( Debug.OPTION_SERVICES_LOGGING, String.format( "Failed to create filter '%1$s' for logger '%2$s'", value, name_ ), e ); //$NON-NLS-1$
            }
        }
    ...
    List<Handler> getHandlers()
    {
        ...
        if( value != null )
        {
            for( final String name : value.split( "[,\\s]+" ) ) //$NON-NLS-1$
            {
                ...
                {
                    Debug.getDefault().trace( Debug.OPTION_SERVICES_LOGGING, String.format( "Illegal handler name specified for logger '%1$s'", name_ ), e ); //$NON-NLS-1$
                }
                catch( final ComponentException e )
                {
                    Debug.getDefault().trace( Debug.OPTION_SERVICES_LOGGING, String.format( "Failed to create handler '%1$s' for logger '%2$s'", name, name_ ), e ); //$NON-NLS-1$
                }
            }
        }
        ...
        final Level defaultValue )
    {
        ...
        {
            Debug.getDefault().trace( Debug.OPTION_SERVICES_LOGGING, String.format( "Failed to parse level for logger '%1$s'", name_ ), e ); //$NON-NLS-1$
        }
