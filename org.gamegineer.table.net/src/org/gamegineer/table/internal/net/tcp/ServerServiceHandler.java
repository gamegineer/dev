    ...
    implements IServiceHandler<SelectableChannel, SelectionKey>
{
    ...
    /** The server service handler state. */
    @GuardedBy( "lock_" )
    ...
    public void close()
    {
        synchronized( lock_ )
        {
            if( state_ == State.OPENED )
            {
                ...
                {
                    Loggers.getDefaultLogger().log( Level.SEVERE, Messages.ServerServiceHandler_close_ioError, e );
                }
                finally
                {
                    channel_ = null;
        ...
        final SelectableChannel handle )
    {
        ...
        synchronized( lock_ )
        {
            assertStateLegal( state_ == State.PRISTINE, Messages.ServerServiceHandler_state_notPristine );
    ...
    /**
     * The possible server service handler states.
     */
    private enum State
    {
        /** The server service handler has never been used. */
        PRISTINE,

        /** The server service handler is open. */
        OPENED,

        /** The server service handler is closed. */
        CLOSED;
    }
}
